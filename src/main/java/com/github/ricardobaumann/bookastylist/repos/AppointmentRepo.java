...
public interface AppointmentRepo extends CrudRepository<Appointment, Long> {
    ...
    @Query(nativeQuery = true,
            value = "select s.id, s.name, s.email from stylist s " +
                    "   where not exists (" +
                    "       select 1 from appointment a " +
                    "         where a.stylist_id = s.id" +
                    "          and a.date = ?1 and a.slot_number = ?2" +
                    "   )")
    List<Object[]> queryStylists(LocalDate adate, Integer slotNumber);
    ...
    default List<Stylist> findAvailableStylistsFor(LocalDate date, Integer slotNumber) {
        return queryStylists(date, slotNumber).stream()
                .map(objects -> new Stylist(((BigInteger) objects[0]).longValue(), (String) objects[1], (String) objects[2]))
